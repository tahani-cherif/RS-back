...
exports.findEtablissementIdByStockPoubelleId = asyncHandler(async (req, res, next) => {

    const stockPoubelle = await Stock_poubelle.findByPk(req.params.id, {
      include:[
    ...
    if (!stockPoubelle) {
      return next(
        new ApiError(`Stock_poubelle not found for this id ${id}`, 404)
      );
    }

    const etablissementId = stockPoubelle.Stock_blocPoubelle.Etage_etablissement.Bloc_etablissement.Etablissement.id;
     res.status(200).json({ etablissementId });
...
exports.findPoubelleByCamion = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
  var test = [];
  var etabl = [];
  var blocpoubelle = [];
  ...
  for (const etab of etablissement) {
    etabl.push(etab.Bloc_etablissements);
  }
  const blockEtabArray = etabl.flat();

  for (const etab of blockEtabArray) {
...
exports.getStock_poubelles = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json({ results: stock_poubelle.length, data: stock_poubelle });
});
...
exports.updateStock_poubelle = asyncHandler(async (req, res, next) => {
  ...
  if (req.body.etat) {
    ...
    document = {
      ...
      etat:
        stockPoubelle.capacite === 0
          ? 0
          : (100 * req.body.quantite_actuel) / stockPoubelle.capacite,
    };
  }
  if (req.body.etat > 80) {
    ...
    await Planning.create({
      ...
      validation: false,
      statut: "en cours",
      type_poubelle: stockPoubelle.type_poubelle,
      EtablissementId:
        stockPoubelle.Stock_blocPoubelle.Etage_etablissement.Bloc_etablissement
          .Etablissement.id,
      CamionId:
        stockPoubelle.Stock_blocPoubelle.Etage_etablissement.Bloc_etablissement
          .Etablissement.CamionId,
    });
  }
